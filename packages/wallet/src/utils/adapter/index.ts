...
import {
  DEVNET_REVIBASE_LOOKUP_TABLE_ADDRESS,
  DEVNET_RPC_ENDPOINT,
...
function getAddressLookUpTableAddress() {
  return getSolanaRpcEndpoint() === DEVNET_RPC_ENDPOINT
    ? DEVNET_REVIBASE_LOOKUP_TABLE_ADDRESS
    : REVIBASE_LOOKUP_TABLE_ADDRESS;
}
...
export function getAddressByLookUpTable(): AddressesByLookupTableAddress {
  return {
    [address(getAddressLookUpTableAddress())]: [
