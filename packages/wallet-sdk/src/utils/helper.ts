...
export async function getDelegateAddress(walletAddress: string | Secp256r1Key) {
...
export async function getSettingsFromInitialMember(
  walletAddress: string | Secp256r1Key
) {
  if (isAddress(walletAddress.toString())) {
...
export async function getMultiWalletFromSettings(settings: string) {
  const [multiWallet] = await getProgramDerivedAddress({
    programAddress: MULTI_WALLET_PROGRAM_ADDRESS,
    seeds: [
      getUtf8Encoder().encode("multi_wallet"),
      getAddressEncoder().encode(address(settings)),
