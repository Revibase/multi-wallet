...
export class CompiledKeys {
  ...
  static compile(
    ...
    for (const ix of instructions) {
      ...
      getOrInsertDefault(ix.programAddress.toString()).isInvoked = false;
      for (const accountMeta of ix.accounts ?? []) {
        const keyMeta = getOrInsertDefault(accountMeta.address.toString());
        keyMeta.isSigner ||=
  ...
  getMessageComponents(): [
    ...
    const staticAccountKeys = [
      ...writableSigners.map(([key]) => address(key)),
      ...readonlySigners.map(([key]) => address(key)),
      ...writableNonSigners.map(([key]) => address(key)),
      ...readonlyNonSigners.map(([key]) => address(key)),
    ];
  ...
  extractTableLookup(lookupTableAddresses: [string, Address[]]) {
    ...
    return {
      addressTableLookup: {
        lookupTableAddress: address(lookupTableAddresses[0]),
        writableIndices,
        readableIndices,
      },
      drainedKeys: {
