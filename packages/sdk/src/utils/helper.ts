...
export async function getSettingsFromCreateKey(createKey: Address) {
...
export async function getTransactionBufferAddress(
  ...
  if (isAddress(getPubkeyString(creator))) {
    const [transactionBuffer] = await getProgramDerivedAddress({
      ...
      seeds: [
        ...
        getAddressEncoder().encode(address(getPubkeyString(creator))),
...
export function getMemberKeyString(memberKey: MemberKey) {
